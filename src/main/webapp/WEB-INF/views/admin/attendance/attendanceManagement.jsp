...
<div class="w-100">
	<div class="min-width-340">
		<div class="px-9 pt-4 pb-3">
			<a href="/admin/attendance/management">
						...
						<c:otherwise>
							<c:forEach items="${allDataList}" var="dclzStatus">
								...
								<tr id="detail" onclick="detailOpen(${dclzStatus.emplNo},'${dclzStatus.emplNm}','${dclzStatus.clsfNm}','${dclzStatus.deptNm}')" style="cursor: pointer;">
									...
									<td>
										<a href="javascript:void(0)" onclick="modify('${dclzStatus.emplNm}','${dclzStatus.clsfNm}',${dclzStatus.dclzNo},'${dclzStatus.attendDt}','${dclzStatus.lvffcDt}', event)">
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="updateForm" action="/admin/attendance/update" method="post">
				<input type="hidden" id="dclzNo" name="dclzNo" value="">
					<div class="mb-3 row">
						<label for="recipient-name" class="col-sm-3 col-form-label text-center" style="font-weight: lighter; font-size: 15px;">출근 시간</label>
						<div class="col-sm-9">
							<div class="form-group">
								<input type="datetime-local" class="form-control" id="attendDt" name="attendDtString" value="">
							</div>
						</div>
					</div>
					<div class="mb-3 row">
						<label for="recipient-name" class="col-sm-3 col-form-label text-center" style="font-weight: lighter; font-size: 15px;">퇴근 시간</label>
						<div class="col-sm-9">
							<div class="form-group">
								<input type="datetime-local" class="form-control" id="lvffcDt" name="lvffcDtString" value="">
							</div>
						</div>
					</div>
					<sec:csrfInput />
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-danger" data-bs-dismiss="modal" id="updateCancle">취소</button>
			</div>
		</div>
	</div>
</div>
...
var updateBtn = $("#updateBtn"); // 수정
var updateForm = $("#updateForm"); // 수정 폼
var updateCancle = $("#updateCancle"); // 모달 취소
...
//수정
function modify(emplNm, clsfNm, dclzNo, attendDt, lvffcDt, event) {
	event.stopPropagation(); // 부모요소 이벤트 막기
	
	var formattedDateAttendDt = updateFormattedDate(attendDt);
	var formattedDateLvffcDt = updateFormattedDate(lvffcDt);
	// input 요소의 값을 설정
	$("#exampleModalLabel1").text(emplNm+" "+clsfNm);
	$('#dclzNo').val(dclzNo);
	$('#attendDt').val(formattedDateAttendDt);
	$('#lvffcDt').val(formattedDateLvffcDt);
...
function updateFormattedDate(time) {
	// 주어진 문자열을 공백을 기준으로 쪼갠 후 각 구성 요소 추출
	...
	// yyyy-MM-ddThh:mm 형식의 문자열로 조합
	var formattedDate = year + "-" + monthNumber + "-" + day + " " + time;
